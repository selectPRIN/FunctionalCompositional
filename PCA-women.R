...
for(i in 1:n){
  dati[[i]] = matrix(NA,TT,c)
    for(j in 1:c){
      for(k in 1:TT){
        v = dat[dat$Country==nations[i]&dat$Year==start.year-1+k&dat$Sex==2&dat$CauseM==j,]
...
si      # we consider 6 groups

# Clustering
set.seed(1)
cl.spec <- matrix(NA,n,1000)
spec.centr <- array(NA, c(6,dim(data.proj)[2],1000))
for(i in 1:1000){
  cc4 <- specc(data.proj, 6)
...
g2 = m+cntrs[2,1,1]*(pc1)+cntrs[2,2,1]*(pc2)+cntrs[2,3,1]*(pc3)+cntrs[2,4,1]*(pc4)
...
g6 = m+cntrs[4,1,1]*(pc1)+cntrs[4,2,1]*(pc2)+cntrs[4,3,1]*(pc3)+cntrs[4,4,1]*(pc4)
g6 <- t(apply(t(apply(t(g6),1,exp)),2, function(x) x/sum(x)))
g5 = m+cntrs[5,1,1]*(pc1)+cntrs[5,2,1]*(pc2)+cntrs[5,3,1]*(pc3)+cntrs[5,4,1]*(pc4)
g5 <- t(apply(t(apply(t(g5),1,exp)),2, function(x) x/sum(x)))
g3 = m+cntrs[6,1,1]*(pc1)+cntrs[6,2,1]*(pc2)+cntrs[6,3,1]*(pc3)+cntrs[6,4,1]*(pc4)
g3 <- t(apply(t(apply(t(g3),1,exp)),2, function(x) x/sum(x)))


save.image(file="results_women.RData")
